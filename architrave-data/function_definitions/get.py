    ...
    for key, value in data.items():
        ...
        if place != None:
            ...
            if place.location.geo.text != "":
                ...
                # one time, a slash was used instead of a comma, so we replace
                # that but we can't assign to the .text attribute, it seems

                value["geo"]["lat"] = float(place.location.geo.text.replace(' /', ',').split(",")[0])
                value["geo"]["long"] = float(place.location.geo.text.replace(' /', ',').split(",")[1])
